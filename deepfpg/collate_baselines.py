...
collate_results.py does for DFPG stats)."""
...
from collate_results import (parse_name_with_nums, name_to_size)
...
def get_domains_problems(baseline_dict):
    ...
    def group_key(thing):
        strs = (t for t in thing[0] if isinstance(t, (str, bytes)))
        return (len(thing), ) + tuple(strs)
...
def join_results(baseline_dict, dom_name, prob_names, planner_name):
    ...
    for prob_name in prob_names:
        ...
        if planner_name not in prob_results:
            ...
            continue
        # XXX how long did it take to do... wait, crap!
        # I have 30 runs from Felipe, but ~100 runs on each of my own problems.
        # Need to equalise run count! Could divide mine by 3.3, but I think
        # it's safer to actually re-run the experiments.
