#!/usr/bin/env python3
"""Collate results for DFPG, from `run_experiment.py`."""
...
def get_run_info(run_dir):
    ...
    return term_stat, elapsed_time, cmdline
...
def parse_results_dir(results_dir):
    ...
    for subdir in subdirs:
        ...
        try:
            ...
            term_stat, elapsed_time, cmdline = get_run_info(run_dir)
        ...
        is_train = skip or not results_dict['no_train']

        # things of interest: no_train, problem, trials, successes,
        # mean_makespan, turn_limit
        if is_train:
            if seen_train_dir:
                raise Exception("Two train dirs (%s and %s)---delete one!"
...
parser.add_argument(
    '--write-json-to',
    default=None,
    help='write .json to this file as well as stdout')
parser.add_argument(
    'results_dir',
    metavar='results-dir',
    nargs='+',
...
def main():
    ...
    for results_dir in args.results_dir:
        ...
        print('== JSON RESULTS')
        print('-' * 80)
        print_expt_json(results)
        print('-' * 80)
        if args.write_json_to:
            print('Writing JSON to "%s"' % args.write_json_to)
            with open(args.write_json_to, 'w') as fp:
