...
def _kernprof_options():
    ...
    # explainer: kernprof 2.0 uses execfile(<fpg.py path>, locals(), locals())
    # to run scripts. That means that fpg.py gets access to all the internal
    # stuff that kernprof uses to set up scripts---including the 'options'
    # object containing all command line options! We save those globals in
    # fpg.py so we can access them later :-)
    return _fpg_main_globals['options']  # noqa
