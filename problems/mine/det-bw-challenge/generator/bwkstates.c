...
char *argv[];
{
    ...
    int initial;          /* Number of towers in odd rank (initial) states */
    int goal;             /* Number of towers in even rank (goal) states */
    int x;
    ...
    initial = -1;                                   /* Default */
    goal = -1;                                      /* Default */
    get_options(argc,argv,&(sigma->N),
		&S,&seed,&initial,&goal);           /* Read command line */
    if (!(initial+1) || !(goal+1)) {
    ...
    while (S--) {                                   /* Loop S times */
      x++;
      if (x%2) make_state(sigma,ratio,initial);	
      else make_state(sigma,ratio,goal);
      print_state(sigma);
    }
    printf("0\n");                                  /* EOF marker */
}
...
void get_options(int argc, char *argv[], int *size, int *nstates, 
		 long *seed, int *initial, int *goal)
{
    int option;
    extern char *optarg;

    while ((option = getopt (argc, argv, "n:r:s:i:g:")) != -1)
	switch( option ) {
	  ...
	  case 's': sscanf(optarg,"%d",nstates);
	    break;
	  case 'i': sscanf(optarg,"%d",initial);
	    break;
	  case 'g': sscanf(optarg,"%d",goal);
	    break;
	}
    if (size<0 || *size >SZ) {
	...
	fprintf(stderr, "Bad -i option: number of towers %d out of range\n\n", 
		*initial);
	exit(2);
    }
    if (*goal > *size || (!(*goal) && size)) {
	fprintf(stderr, "Bad -g option: number of towers %d out of range\n\n", 
		*goal);
	exit(2);
    }
}
...
/* ST 15-4 new parameter */
void make_state(state sigma, float ratio[], int ntt)
{
    int x;
	...
	else p = ((float)(ntt - sigma->nrt))/((float) (sigma->nft +1));
        /* ST 15-4 removed the <=, because anyway r is strictly lower than 1.
           Furthermore r can be 0.0, in which case, if p is 0.0, I don't
           want the tower to be put on the table (this is anoying when you
           have already reached the total number of towers allowed !!) */
